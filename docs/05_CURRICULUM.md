...
| C3 | **Define: create an observable** | Create a derived quantity without confusing transformation with selection. | `Define` | **NEXT** — promote the existing `practice/code` Define drill into a full curricular lesson rather than rebuilding the mechanic. |
...
The default next lesson on this roadmap is **C3 — Define: create an observable**.
