...
Implement the smallest experience that reaches the learner outcome and satisfies the lesson contract. Do not implement the following lesson in the same task unless the user explicitly asks.
...
Then run the relevant build/tests and interaction checks.
